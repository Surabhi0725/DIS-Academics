...
public class FileStorageServiceImpl implements FileStorageService {
    ...
    @Override
    public void saveMagazine(String magazineId, MultipartFile file, int size)
    {
        ...
        try
        {
            Files.copy(file.getInputStream(), this.root.resolve(magazine.getTopic()+"_"+magazine.getDate()+"_image_"+Integer.toString(this.count) + ".jpg"));
            this.count -= 1;
        } catch(Exception e)
        {
            throw new RuntimeException("Could not upload the file. Error: "+e.getMessage());
    ...
    @Override
    public List<FileInfo> filterImagesMagazine(String magazineId, List<FileInfo> fileInfos)
    {
        MagazineDetails magazine = magazineRepository.findByMagazineId(magazineId);
        String visit = magazine.getTopic()+"_"+magazine.getDate();
        int count = 1;
        List<FileInfo> output = new ArrayList<>();
