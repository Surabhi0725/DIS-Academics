...
public class ExpertLectureController {
	...
	@PostMapping(path = RestAPI.EDIT_EXPERT, produces = "application/json")
	...
	@ApiOperation(value = "Get all expert lectures", response = ExpertLecturesResponse.class, httpMethod = "GET", produces = "application/json")
	@GetMapping(path=RestAPI.GET_ALL_EXPERT_LECTURES, produces = "application/json")
	public ResponseEntity<?> getAllExpertLectures(HttpServletRequest request){
		if (!jwtResolver.getUserTypeFromJwtToken(request.getHeader("Authorization")).equals("student"))
			return new ResponseEntity<List<ExpertLectureDetails>>(expertLectureService.getAllExpertLectures(), HttpStatus.OK);
		else
			return new ResponseEntity<ResponseMessage>(new ResponseMessage("Sorry, you are not allowed to use this service."), HttpStatus.BAD_REQUEST);
	}
	@ApiOperation(value = "Get all experts", response = ExpertDetails.class, httpMethod = "GET", produces = "application/json")
	@GetMapping(path=RestAPI.GET_ALL_EXPERTS, produces = "application/json")
	public ResponseEntity<?> getAllExperts(HttpServletRequest request){
		if (!jwtResolver.getUserTypeFromJwtToken(request.getHeader("Authorization")).equals("student"))
			return new ResponseEntity<List<ExpertDetails>>(expertLectureService.getAllExperts(), HttpStatus.OK);
		else
			return new ResponseEntity<ResponseMessage>(new ResponseMessage("Sorry, you are not allowed to use this service."), HttpStatus.BAD_REQUEST);
	}
	...
	public ResponseEntity<?> updatePaymentStatusAndRemarks(HttpServletRequest request, @PathVariable("expertLectureId") String expertLectureId,
														   @RequestParam(value = "payment_status") String paymentStatus, @RequestParam(value = "remarks") String remarks)
	{
		if (!jwtResolver.getUserTypeFromJwtToken(request.getHeader("Authorization")).equals("student"))
	...
	public ResponseEntity<?> uploadImages(HttpServletRequest request,
										  @PathVariable("id") String expertLectureId, @RequestParam("photos") MultipartFile[] photos)
	{
		if (!jwtResolver.getUserTypeFromJwtToken(request.getHeader("Authorization")).equals("student"))
		{
			String message = "";
